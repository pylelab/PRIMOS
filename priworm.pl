...
#
#  WORMBASE
#  Carlos M Duarte, Copyright 2001
...
$dir = <STDIN>;
chop ($dir);
# print STDOUT "Do you want separate files for each worm fragment?\n";
# $FRAG = <STDIN>;
# chop ($FRAG);
...
for ($i = 1; $i <= $NUMCHN; $i++) {
	...
	$START = 0;
	$WORM = $FILE . "_" . $i . "_worm.txt";
	open (WORMOUT, ">$WORM") || die "couldn't open $WORM\n";
	$RESCOUNT -= 2;
	$TRES = $ENDRES[$i] - $STRTRES[$i] + 1;
	ROLL:for ($j = 1; $j <= $TRES; $j++) {
		$NRES = $j + $STRTRES[$i] - 1;
		if ($j == 1 || $j == $TRES || $RNA[$NRES] == 0 || $RNA[$NRES-1] == 0 || $RNA[$NRES+1] == 0) {
			...
			$BFACT[$NRES] = (substr($ATOM[$C1NAM],60,6));
			&MEAS(@P1,@C1);
			$DISPC[$NRES] = $DIST;
			&DIHED(@P1, @O5, @C5, @C1, 1);
			$BETA[$NRES] = $TOR;
			...
			$ALPHA[$NRES] = $TOR;
			if ($CPCP[$NRES] != 0 && $PCPC[$NRES] != 0) {
				if ($BFAC) {
					printf WORMOUT "%-7s\t%-3d\t%-7s\t%-4s\t%6.1f\t%6.1f\t%6.2f\n", $FILE, $i, $RESNAM[$NRES], $RESTYP[$NRES], $CPCP[$NRES], $PCPC[$NRES], $BFACT[$NRES];
					}
				else {
					printf WORMOUT "%-7s\t%-3d\t%-7s\t%-4s\t%6.1f\t%6.1f\n", $FILE, $i, $RESNAM[$NRES], $RESTYP[$NRES], $CPCP[$NRES], $PCPC[$NRES];
					}
				}
			else {
				print "Residue $NRES is a 0-0\n";
				}
			}
		}
	}
